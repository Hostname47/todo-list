...
function Messages({ status, message, type, switchMessageStatus }) {
    ...
    <div>
      { status && (
        <div className={ sectionClass }>
          ...
          <button className='close' onClick={ () => switchMessageStatus(false) }>
...
const mapDispatchToProps = dispatch => {
  return {
    switchMessageStatus: status => dispatch(switchMessageStatus(status))
  }
}
